#pragma once
#include <algorithm>
#include <vector>
#include <string>

namespace isolated
{
    inline std::vector<uint8_t> isoBytes =
    {
        
    };

    // Replace bytes in the vector
    inline void replaceByteSequence(std::vector<uint8_t>& byteVec, const std::vector<uint8_t>& oldSeq, const std::vector<uint8_t>& newSeq)
    {
        size_t pos = 0;
        while ((pos = std::search(byteVec.begin() + pos, byteVec.end(), oldSeq.begin(), oldSeq.end()) - byteVec.begin()) != byteVec.size())
        {
            byteVec.erase(byteVec.begin() + pos, byteVec.begin() + pos + oldSeq.size());
            byteVec.insert(byteVec.begin() + pos, newSeq.begin(), newSeq.end());
            pos += newSeq.size(); // Move past the newly inserted sequence to avoid re-matching the replaced bytes.
        }
    }
}
